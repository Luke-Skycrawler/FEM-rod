...
@ti.func
def piona(F):
    log_J = ti.log(F.determinant())
    F_inv_T = F.transpose().inverse()
    return mu * (F - F_inv_T)  + lam * log_J * F_inv_T

@ti.kernel
def update_U():
    for i in range(NV):
        f[i] = ti.Vector.zero(float, 2)
    ...
    for i in range(NF):
        ...
        F[i] = D_i @ B[i]
        P = piona(F[i])
        assert all(P == ti.Matrix.zero(float, 2,2))
        H = - W[i] * P @ (B[i].transpose())
        # H = - (dx * 0.25) ** 2 * 0.5 * P @ B[i].transpose()
...
def init_pos():
    ...
    for i in range(NF):
        ...
        B[i] = B_i_inv.inverse()
        W[i] = ti.abs(B_i_inv.determinant()) * 0.5
